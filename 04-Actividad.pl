% -------------------------- Ejercicio Simpsons (01) con Listas -------------------------- %

% Definimos las familias como listas
    familia([abraham, mona, [homero, herbert]]).
    familia([homero, marge, [bart, lisa, maggie]]).
    familia([clancy, jacqueline, [marge, patty, selma]]).
    familia([selma, selma, [ling]]).

% Definimos las relaciones familiares
    padre_de(Padre, Hijo) :- familia([Padre, _, Hijos]), member(Hijo, Hijos).
    madre_de(Madre, Hijo) :- familia([_, Madre, Hijos]), member(Hijo, Hijos).

% Definimos el género utilizando listas
...
% -------------------------- Ejercicio Simpsons (02) con Listas y Estructuras -------------------------- %
